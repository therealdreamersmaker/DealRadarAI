...
async function step1_selectTopZipCodes() {
  ...
  // Simulated market intelligence — in production, call BatchData/RentCast API
  const mockZipCodes = [
...
async function step2_extractDistressedProperties(zipCodes) {
  ...
  for (const zipInfo of zipCodes) {
    ...
    // Simulated data — in production: call BatchData API with absenteeOwner + highEquity + preForeclosure filters
    for (let i = 0; i < batchSize; i++) {
      const streetNum = 1000 + Math.floor(Math.random() * 8000);
      const streets = ['Oak Ave', 'Maple St', 'Pine Rd', 'Cedar Blvd', 'Elm Dr', 'Birch Ln'];
      ...
      const arv = 120000 + Math.floor(Math.random() * 200000);
...
async function step3_skipTrace(properties) {
  ...
  while (enriched.length < 100 && idx < properties.length) {
    ...
    // Simulated skip-trace — in production: call BatchData skip-trace endpoint
    await rateLimitedRequest(() => {}, 100);

    const firstNames = ['James', 'Michael', 'Robert', 'David', 'William', 'Maria', 'Linda', 'Patricia', 'Barbara', 'Susan'];
    ...
    const areaCode = ['404', '713', '205', '843', '225'][Math.floor(Math.random() * 5)];
    const phoneNum = `${Math.floor(Math.random() * 900) + 100}${Math.floor(Math.random() * 9000) + 1000}`;
    const isMLS = Math.random() > 0.5;
    const agentFirstNames = ['Sarah', 'John', 'Emily', 'Chris', 'Amanda'];
    const agentLastNames = ['Parker', 'Smith', 'Chen', 'Williams', 'Jones'];
    ...
    enriched.push({
      ...
      agentName: isMLS
        ? `${agentFirstNames[Math.floor(Math.random() * agentFirstNames.length)]} ${agentLastNames[Math.floor(Math.random() * agentLastNames.length)]}`
        : 'Off-Market',
...
module.exports = { runAutopilot, getState, EXPORTS_DIR };
